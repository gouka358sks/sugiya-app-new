...
export default function StaffHoursModal({ dateStr, dayHours, onSave, onClose }) {
  ...
  const handleSelectStaff = (staffId, name) => {
    const regularShift = shifts.find(sh => sh.staffName === name && sh.dayOfWeek === dayOfWeek);
    setAddForm({
      staffId, name,
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal sh-modal" onClick={ev => ev.stopPropagation()}>
            ...
            entries.map(entry => (
              <div key={entry.id} className="sh-entry sh-active">
                <div className="sh-entry-top">
                  ...
                  <button className="sh-delete-btn" onClick={() => handleDelete(entry.id)} title="削除">✕</button>
                </div>
                <div className="sh-times">
                  <input
                    type="time" value={entry.startTime}
                    onChange={e => handleTimeChange(entry.id, 'startTime', e.target.value)}
                  />
                  <span className="sh-sep">〜</span>
                  <input
                    type="time" value={entry.endTime}
                    onChange={e => handleTimeChange(entry.id, 'endTime', e.target.value)}
                  />
                  <span className="sh-hrs">{calcHours(entry.startTime, entry.endTime).toFixed(1)}h</span>
                </div>
              </div>
            ))
          )}
          ...
          {!showAddForm ? (
            <>
              ...
              <div className="sh-staff-buttons">
                {staff.map(s => (
                  <button key={s.id} className="sh-staff-btn" onClick={() => handleSelectStaff(s.id, s.name)}>
                    ＋ {s.name}
                  </button>
                ))}
                {staff.length === 0 && (
                  <button className="sh-staff-btn sh-manual-btn" onClick={() => { setAddForm(EMPTY_ADD); setShowAddForm(true); }}>
                    ＋ 手動で追加
                  </button>
                )}
              </div>
            ...
            <div className="sh-add-form">
              {!addForm.name && (
                <div className="form-group" style={{ marginBottom: 8 }}>
                  <label>スタッフ名</label>
                  <input
                    type="text" list="sh-staff-datalist" value={addForm.name}
                    onChange={e => setAddForm(p => ({ ...p, name: e.target.value, staffId: '' }))}
                    placeholder="名前を入力" autoFocus
                  />
              ...
              {addForm.name && (
                <p className="sh-add-who"><strong>{addForm.name}</strong> の時間帯を入力:</p>
              )}
              <div className="sh-times">
                ...
                <span className="sh-hrs">
                  {addForm.startTime && addForm.endTime ? calcHours(addForm.startTime, addForm.endTime).toFixed(1) + 'h' : ''}
                </span>
              </div>
              <div className="sh-add-actions">
                <button className="btn-secondary" onClick={() => { setShowAddForm(false); setAddForm(EMPTY_ADD); }}>キャンセル</button>
                <button className="btn-primary" onClick={handleAdd} disabled={!addForm.name}>追加</button>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="modal-actions">
          ...
          <button className="btn-primary" onClick={() => onSave(dateStr, entries)}>保存</button>
        </div>
      </div>
    </div>
  );
